...
const SERVER_URL = 'http://localhost:3001'; // Your backend server
...
async function fetchFromCache(domain) {
  const res = await fetch(`${SERVER_URL}/domain/${domain}`);
  return await res.json();
}

async function storeInCache(domain, score, suggestions) {
  await fetch(`${SERVER_URL}/domain`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ domain, score, suggestions })
  });
}
...
chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
  ...
  let cvssScore, suggestions;
  const cached = await fetchFromCache(domain);

  if (cached && cached.score !== undefined) {
    cvssScore = cached.score;
    suggestions = cached.suggestions;
  } else {
    cvssScore = await getNVDScore(domain);
    suggestions = await getChatGPTRecommendations(domain);
    await storeInCache(domain, cvssScore, suggestions);
  }
